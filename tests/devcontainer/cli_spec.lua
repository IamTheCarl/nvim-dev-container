...
describe("cli.up / cli.exec / cli.recreate arg construction", function()
  ...
  it("cli.exec places --remote-env flags before the command", function()
    subject.exec("abc123def456", "echo", { "hi" }, { remote_env = { SHELL = "/bin/bash" } })
    local remote_env_pos = index_of(captured.args, "--remote-env")
    local cmd_pos = index_of(captured.args, "echo")
    assert.is_not_nil(remote_env_pos)
    assert.is_not_nil(cmd_pos)
    assert.is_true(remote_env_pos < cmd_pos, "--remote-env must appear before the command arg")
  end)
